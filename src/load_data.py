...
class afm_data_generator():
        ...
        if self.args.skill_wise[0]=="True":
            if (not os.path.exists(self.args.source_path+self.args.dataset[0]+"/SkillWise/"+"skill_index.csv")) or (not os.path.exists(self.args.source_path+self.args.dataset[0]+"/SkillWise/"+"problem_index.csv")):
                ...
                for i,j in d_skill.items():
                    skills.append(i)
                    index.append(j)
                if not os.path.exists(self.args.source_path+self.args.dataset[0]+"/SkillWise/"):
                    os.makedirs(self.args.source_path+self.args.dataset[0]+"/SkillWise/")
                pd.DataFrame({"skills":skills, "index":index}).to_csv(self.args.source_path+self.args.dataset[0]+"/SkillWise/"+"skill_index.csv", sep=",", index=False)
    ...
    def main(self):
        ...
        #print('::',self.args)
        #print('??',df_user_responses)
        #print('!!!',original_data)
        self.df_user_responses = df_user_responses

        temp = "" if self.args.puser[0] == "orig" else "sub"
        self.user_train = set(pd.read_csv(self.args.source_path+"datasets/" +self.args.dataset[0]+"/Users/"+temp+"train.csv", header=None,sep='\t')[0].map(str))
        self.user_test = set(pd.read_csv(self.args.source_path+"datasets/"+self.args.dataset[0]+"/Users/"+temp+"test.csv", header=None,sep='\t')[0].map(str))
